...
public class ThompsonTest {
	...
	public void testSimpleComposeNFA() {
		NFA nfa = Thompson.transfer(Arrays.asList("a(b)c(de)"));
		nfa = addEmptyHead(nfa);
		// nfa = head--ϵ-->s0--a-->s1--ϵ-->s2--b-->s3--ϵ-->s4--c-->s5--ϵ-->s6--d-->s7--ϵ-->s8--e-->s9
		Status s = nfa.getFirst();
		for (int i = 0; i < 5; i++) {
	...
	public void testRepeatNFA() {
		...
		//                               +-----------ϵ--------------↑
		NFA nfa = Thompson.transfer(Arrays.asList("ab*c+"));
		Status s0 = nfa.getFirst();
		assertEquals(Sets.newHashSet('a'), s0.getEdges().get(0).getAcceptChars());
		Status s1 = s0.getEdges().get(0).getTargetStatus();
		assertTrue(s1.getEdges().get(0).isEpsilon());
		Status s2 = s1.getEdges().get(0).getTargetStatus();
		List<Edge> e2 = s2.getEdges();
		assertEquals(2, e2.size());
		// 以下两个变量的 0 / 1 取决于构造 NFA 时添加边的顺序
		Edge e2S3 = e2.get(0);
		Edge e2S5 = e2.get(1);
		Status s3 = e2S3.getTargetStatus();
		assertEquals(Sets.newHashSet('b'), s3.getEdges().get(0).getAcceptChars());
		Status s4 = s3.getEdges().get(0).getTargetStatus();
		...
		Status s5 = e4S5.getTargetStatus();
		assertSame(s5, e2S5.getTargetStatus());
		assertSame(s3, e4S3.getTargetStatus());
		assertTrue(s5.getEdges().get(0).isEpsilon());
		Status s6 = s5.getEdges().get(0).getTargetStatus();
		assertTrue(s6.getEdges().get(0).isEpsilon());
		Status s7 = s6.getEdges().get(0).getTargetStatus();
		assertEquals(Sets.newHashSet('c'), s7.getEdges().get(0).getAcceptChars());
		Status s8 = s7.getEdges().get(0).getTargetStatus();
	...
	public void testInputSet() {
		...
		//                                                                      +---------------ϵ--------------↑
		NFA nfa = Thompson.transfer(Arrays.asList("a[xy\\d][a-z]*"));
		Status s0 = nfa.getFirst();
		assertEquals(Sets.newHashSet('a'), s0.getEdges().get(0).getAcceptChars());
		Status s1 = s0.getEdges().get(0).getTargetStatus();
		assertTrue(s1.getEdges().get(0).isEpsilon());
		...
		assertEquals(xy0_9, s2.getEdges().get(0).getAcceptChars());
		Status s3 = s2.getEdges().get(0).getTargetStatus();
		...
		Edge e4S7 = e4.get(1);
		Status s5 = e4S5.getTargetStatus();
		assertEquals(Char.lowers(), s5.getEdges().get(0).getAcceptChars());
		Status s6 = s5.getEdges().get(0).getTargetStatus();
		...
		Status s7 = e6S7.getTargetStatus();
	...
	public void testComposeRepeatNFA() {
		...
		//                                 +             ↑--------------------------------ϵ------------------------------------+              ↑
		//                                 +-----------------------------------------ϵ-----------------------------------------------+
		NFA nfa = Thompson.transfer(Arrays.asList("a([bc]|d|e)*"));
		Status s0 = nfa.getFirst();
		assertEquals(Sets.newHashSet('a'), s0.getEdges().get(0).getAcceptChars());
		Status s1 = s0.getEdges().get(0).getTargetStatus();
		assertTrue(s1.getEdges().get(0).isEpsilon());
		Status s12 = s1.getEdges().get(0).getTargetStatus();
		...
		Status s10 = s12.getEdges().get(0).getTargetStatus();
		...
		Status s8 = e10S8.getTargetStatus();
		List<Edge> e8 = s8.getEdges();
		Edge e8S6 = e8.get(0);
		Edge e8S4 = e8.get(1);
		assertTrue(e8S6.isEpsilon());
		assertTrue(e8S4.isEpsilon());
		Status s6 = e8S6.getTargetStatus();
		assertEquals(Sets.newHashSet('e'), s6.getEdges().get(0).getAcceptChars());
		Status s7 = s6.getEdges().get(0).getTargetStatus();
		Status s4 = e8S4.getTargetStatus();
		assertEquals(Sets.newHashSet('d'), s4.getEdges().get(0).getAcceptChars());
		Status s5 = s4.getEdges().get(0).getTargetStatus();
		assertTrue(s7.getEdges().get(0).isEpsilon());
		assertTrue(s5.getEdges().get(0).isEpsilon());
		assertSame(s7.getEdges().get(0).getTargetStatus(), s5.getEdges().get(0).getTargetStatus());
		Status s9 = s7.getEdges().get(0).getTargetStatus();
		Status s2 = e10S2.getTargetStatus();
		assertEquals(Sets.newHashSet('b', 'c'), s2.getEdges().get(0).getAcceptChars());
		Status s3 = s2.getEdges().get(0).getTargetStatus();
		assertTrue(s3.getEdges().get(0).isEpsilon());
		assertTrue(s9.getEdges().get(0).isEpsilon());
		assertSame(s3.getEdges().get(0).getTargetStatus(), s9.getEdges().get(0).getTargetStatus());
		Status s11 = s3.getEdges().get(0).getTargetStatus();
		...
		Status s13 = e11S13.getTargetStatus();
